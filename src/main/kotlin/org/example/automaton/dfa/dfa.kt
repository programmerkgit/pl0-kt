...
class DFA(char: Char = ' ') {
     ...
     * @return copied org.example.automaton.dfa.DFA
     */
    private fun copy(): DFA {
    ...
    private fun initializeDFA(char: Char) {
        when (char) {
            in Regex("[a-zA-Z]") -> {
    ...
    private fun import(nfa: DFA): DFA {
    ...
    private fun merge(nfa: DFA): DFA {
        val copyOfThis = copy()
        copyOfThis.import(nfa)
        return copyOfThis
    }

    operator fun plus(nfa: DFA): DFA {
        /* should copy new nfa */
        val copyOfThis = copy()
